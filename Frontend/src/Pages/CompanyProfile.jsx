...
export default function CompanyProfile() {
    ...
    return companyData ? (
        <div
            ...
            className="min-h-screen   flex items-center justify-center  p-6 flex-1">
            <motion.div
            ...
            >
                <div className="flex flex-col md:flex-row gap-8 ">
                    ...
                    <div className="md:w-1/3 flex flex-col items-center gap-6  bg-white p-4 rounded-md">
                        <div className="relative">
                            <div className="w-44 h-44 rounded-full overflow-hidden border-4 border-black/10 shadow-lg ">
                                ...
                                >
                                    {avatarUrl ? (
                                        ...
                                        <div className="w-full h-full bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white text-3xl shadow-md">
                                            <User />
                                        </div>
                                    )}
                                </div>
                            ...
                            {/* mini shadow ring */}
                            <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 bg-white/6 px-4 py-4 rounded-full text-sm backdrop-blur-sm ">
                                <span className="text-xs text-white/90">برای تغییر اندازه عکس را درگ کنید</span>
                            </div>
                        </div>
                        ...
                        <div className="w-full flex flex-col gap-3">
                            ...
                            <div className="flex gap-2 justify-center ">
                                <button type="button" onClick={() => setZoom(z => Math.max(0.5, +(z - 0.1).toFixed(2)))} className="px-4 py-2 bg-white/6 rounded-full">-</button>
                                <input aria-label="zoom" type="range" min="0.5" max="3" step="0.01" value={zoom} onChange={e => setZoom(+e.target.value)} className="w-36" />
                                <button type="button" onClick={() => setZoom(z => Math.min(3, +(z + 0.1).toFixed(2)))} className="px-4 py-2 bg-white/6 rounded-full">+</button>
                            </div>
                            ...
                            <div className="flex gap-3 justify-center">
                                ...
                                <button onClick={resetAvatar} type="button" className="px-4 py-2 bg-red-600/80 text-white rounded-full">حذف</button>
                            </div>
                        </div>
                    </div>

                    {/* Right: Form */}
                    <form onSubmit={handleSubmit} className="max-sm:text-sm md:w-2/3 bg-gray-800 p-6 rounded-xl">
                        <div className="flex items-center justify-between">
                            <h2 className="text-md md:text-2xl font-semibold text-white">ویرایش پروفایل</h2>
                            <div className="text-sm text-white/70">دانیال ✨</div>
                        </div>
                            ...
                            <div className="flex flex-col">
                                ...
                                <div className="flex items-center gap-2 bg-white/5 rounded-lg p-2" >
                                    ...
                                    <input
                                        ...
                                        required
                                        type="email"
                                        value={email || companyData.email}
                                        onChange={e => setEmail(e.target.value)} placeholder="ایمیل" className="bg-transparent outline-none text-white/50 placeholder-white/40 w-full" />
                                </div>
                            </div>

                            <div className="flex flex-col">
                                <label className="text-xs text-white/70 mb-2">رمزعبور</label>
                                <div className="flex items-center gap-2 bg-white/5 rounded-lg p-2">
                                    <Lock size={16} className="text-white/70" />
                                    <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="رمز عبور جدید " className="bg-transparent outline-none text-white placeholder-white/40 w-full" />
                                </div>
                            </div>

                            <div className="flex flex-col">
                                <label className="text-xs text-white/70 mb-2">تایید رمزعبور</label>
                                <div className="flex items-center gap-2 bg-white/5 rounded-lg p-2">
                                    <Lock size={16} className="text-white/70" />
                                    <input type="password" value={confirm} onChange={e => setConfirm(e.target.value)} placeholder="تایید رمزعبور" className="bg-transparent outline-none text-white placeholder-white/40 w-full" />
                                </div>
                            </div>
                        </div>
